...
auth.html (131 KB, routed /login /signin), about, blog, case-studies, contractors, downloads, enterprise, escrow, get-started, gate, help, privacy, terms, trust, rewards, rewards-tc, report, report-lead-gen-email — all present. server.js currently only routes auth.html, rewards, rewards-tc and report via the clean-URL map; the rest (about, blog, case-studies, contractors, downloads, enterprise, escrow, get-started, gate, help, privacy, terms, trust) are NOT in server.js's `map` object, so they 404 on `npm start` even though the physical .html files exist and `express.static` would serve them at their `.html` path. **Open item (Phase 1):** add clean-route entries for all of these to server.js.
...
Confirmed still present and unreferenced by any HTML or server.js: `_patch_auth_guard.js`, `_patch_nav_footer.js`, `_patch_rewards.js`, `add-new-chat.js`, `fix-chat.js`, `fix-new-chat-location.js`, `fix-rentals-2.js`, `embed-screenshots.js`, `take-screenshots.js`, `pwa-fix-body.js`, `pwa-gen-icons.js`, `pwa-patch.js`, `upgrade-handover.js`, `upgrade-rentals.js`, `patch-demo-data.js`, `patch-email-cert.js`, `crew-demo-data.js` (referenced-but-missing per §2 — the *source* patch scripts for it are separate and also orphaned), `messaging-team-item-fix.js`, `messaging-ui-mobile.js`, `build-apks.sh`. None of these were removed by the earlier session. All are one-off build/patch scripts, all provably unreferenced, all well under 30 lines' worth of live logic reused elsewhere. **Phase 1 will delete them** (per the autonomous default: unreferenced and small/one-off → delete rather than "kept for review").
...
## 13. Copy Audit (Australian English + AI-tell sweep)

Numbers below are fresh, from a full re-scan of every `.html` and `.md` file (script-based, per-line count):

| Category | Total hits | Notes |
|---|---|---|
| Em dash / en dash (U+2014/U+2013) | 38 | Down from 2,547 in the previous audit — the earlier session's language sweep genuinely landed on the monolith app files. Remaining hits: SETUP_GUIDE.md (15), documentation/SUPABASE_SETUP.html (11), Crew_App_Crew_Member.html (3), 404.html (2), CrewBase_Field_Worker_App.html (2), offline.html (2), Crew_App_Crew_Manager.html (1), Crew_App_Customer_Role.html (1), CLAUDE.md (1, a code-comment reference to this very rule, not user-facing copy — leave as-is). |
| AI-tell words (delve, seamless(ly), leverage-as-verb, unlock, empower, elevate, supercharge) | 535 | Concentrated in Command_Center_Desktop/Tablet.html (133 each) and CrewBase_Dashboard.html (113) — the admin/CrewBase suite was not covered by the earlier sweep, which focused on the three main customer-facing apps. Crew_App_Crew_Member/Customer_Role/Crew_Manager still have residual hits (58/44/26) too. |
| US spellings in prose | 92 | Concentrated in the three main app files (21/20/20); smaller counts scattered across marketing pages. CSS identifiers and DB column names correctly excluded from this count (regex targets whole-word prose terms only, e.g. `organize`, `favor`, `behavior`, `canceled`, not `color:`/`background-color:`). |

The offline banner string (`crew-framework.js` `_initOfflineIndicator`) still contains a real em dash: `'You're offline — some features may not be available.'` — flagged explicitly by this ground rule and confirmed still present. **Phase 1 must fix.**
...
|---|---|---|
| 1 | 404.html/offline.html branded; most em-dash/AU-English sweep on 3 main apps; lang="en-AU" on auth.html | Remove dead script tags (crew-demo-data.js, crew-integration-e2e.js) and cdn-cgi artifact; add missing server.js clean routes; delete orphan scripts; finish AI-tell/US-spelling/em-dash sweep on Command Center + CrewBase_Dashboard + remainder; fix offline banner em dash; Tree Lopping category; SOS button; star ratings; ARIA |
...
| 4 | — | Everything (no per-role manifests, no apps.html, no complaints.html, no icon pipeline, no legal-page footer links, no SEO tags) |
| 5 | — | Everything (no migrations directory beyond CLI temp files) |
| 6 | — | Everything (Zai fully intact, zero CheckVault code) |
| 7 | — | Everything |
| 8 | — | Everything |
| 9 | — | Everything |
